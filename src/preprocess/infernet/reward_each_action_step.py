...
def main(df, action, file_name):
    ...
    infer_reward_model_path = (
        "../sever_results/infer_reward/model/model_{}_{}.h5".format(file_name, i)
    )
    ...
    for st in range(len(infer_buffer)):
        ...
        states_actions = np.reshape(states_actions, (1, max_len, 144))
        inf_rews = model.predict(states_actions)

        states_actions = np.reshape(states_actions, (max_len, 144))
...
if __name__ == "__main__":
    ...
    for file_name in file_names:
        print(file_name)
        df = pd.read_csv(
            "../sever_results/infer_reward/results/nn_inferred_{}_{}.csv".format(
                file_name, i
            ),
            header=0,
        )
        ...
        res.to_csv(
            "results/nn_inferred_{}_all_action.csv".format(file_name), index=False
        )
