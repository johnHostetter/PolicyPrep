...
def getTrace(filename):
    ...
    raw_data['short_critical'] = np.where((raw_data['inferred_rew_aciton_ps'] > positive_thres) | (raw_data['inferred_rew_aciton_we'] > positive_thres) | (raw_data['inferred_rew_aciton_fwe'] > positive_thres) | (raw_data['inferred_rew_aciton_ps'] < negative_thres) | (raw_data['inferred_rew_aciton_we'] < negative_thres) | (raw_data['inferred_rew_aciton_fwe'] < negative_thres), 1, 0)
...
def calculateTarget(model, traces, student_state, q_diff_percentage):
    ...
    Q_pred_np_problem = np.reshape(targets, (len(targets), 3))
...
def main():
    ...
    for iteration in range(101):
        ...
        directory = 'model_modifyBellman/{}/'.format(file_name)
